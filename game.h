...
void evolve(long time_step, const int *current_field, int *new_field, struct Vec2i field_size, struct Vec2i part_size, struct Vec2i threads, struct Kernel kernel);

int is_alive(const int *field, int w, int h, int x, int y);

int calc_surroundings(const int *field, int w, int h, int x, int y, struct Kernel kernel);
